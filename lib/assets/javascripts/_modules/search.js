...
(function ($, Modules) {
  ...
  Modules.Search = function Search () {
    ...
    function attach () {
      ...
      // When focus moves from an elements inside the search results pane to an
      // element outside of it, hide the results pane.
      $searchResults.on('focusout', function (e) {
        var newlyFocusedElement = e.relatedTarget
        if (!$searchResults.get(0).contains(newlyFocusedElement)) {
          hideResults()
        }
      })
