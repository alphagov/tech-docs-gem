...
class ValeLinterReport < LinterReport
    ...
    def set_output_summary
        ...
        @output_summary << "  Errors:      #{error > 0 ? set_output_text_color(error, 'error') : error}"
        @output_summary << "  Warnings:    #{warning > 0 ? set_output_text_color(warning, 'warning') : warning}"
        @output_summary << "  Suggestions: #{suggestion > 0 ? set_output_text_color(suggestion, 'suggestion') : suggestion}"
        @output_summary << "-" * [0, 40].max
    end
    ...
    def set_table_column_widths
      ...
      @table_column_widths = [:file, :line, :severity, :message, :rule].each_with_object({}) do |col, acc|
